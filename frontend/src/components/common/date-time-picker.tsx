...
// ""), the same format the native datetime-local input produced.
...
export function DateTimePicker({
  ...
  const update = (patch: Partial<LocalParts>) => {
    ...
    onChange(formatLocal({ ...base, ...patch }));
  };
  ...
  return (
    ...
    >
      ...
      <PopoverContent
        ...
        ref={popupRef}
        // Start on the selected (or today's) day, ready for arrow keys.
        initialFocus={() =>
          popupRef.current?.querySelector<HTMLElement>('[role="grid"] button[tabindex="0"]') ?? true
        }
      >
        <div className="flex flex-wrap gap-3">
          <Calendar key={calendarKey} selected={selectedDay} onSelect={(d) => update(d)} />
